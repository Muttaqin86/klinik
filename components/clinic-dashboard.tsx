...
type MedicalRecord = { id: string; patientId: string; patientName: string | null; visitDate: string; diagnosis: string; treatment: string; doctor: string; notes: string }
import {
  Activity,
  Bell,
  CalendarDays,
...
export default function ClinicDashboard() {
  ...
  async function loadMedicalRecords() {
    ...
    setMedicalError('')
    try {
      const response = await fetch('/api/medical-records', { cache: 'no-store' })
      const result = await response.json()
      if (!response.ok) throw new Error(result.error)
      setMedicalRecords(result.records)
    } catch (loadError) {
      setMedicalError(loadError instanceof Error ? loadError.message : 'Data rekam medis gagal dimuat.')
    } finally {
  ...
  async function loadPatients() {
    ...
    try {
      ...
      setPatientsError(loadError instanceof Error ? loadError.message : 'Data pasien gagal dimuat.')
    } finally {
  ...
  function selectMenu(label: string) {
    ...
    if (label === 'Pasien') void loadPatients()
    if (label === 'Rekam Medis') void loadMedicalRecords()
  }
  ...
  return <div className="min-h-screen bg-background text-foreground">
    ...
    <div className="lg:pl-64"><header className="sticky top-0 z-10 flex h-20 items-center justify-between border-b border-border bg-background/95 px-5 backdrop-blur md:px-8"><div className="flex items-center gap-3"><button aria-label="Buka menu" onClick={() => setMobileNav(!mobileNav)} className="rounded-lg p-2 text-muted-foreground hover:bg-muted lg:hidden"><Menu size={20} /></button><div><p className="text-sm text-muted-foreground">Senin, 24 Juni 2024</p><h1 className="text-lg font-bold tracking-tight">Selamat pagi, Alya</h1></div></div><div className="flex items-center gap-2 md:gap-4"><button aria-label="Cari" className="hidden rounded-lg p-2.5 text-muted-foreground hover:bg-muted sm:block"><Search size={19} /></button><button aria-label="Notifikasi" className="relative rounded-lg p-2.5 text-muted-foreground hover:bg-muted"><Bell size={19} /><span className="absolute right-2 top-2 size-1.5 rounded-full bg-destructive" /></button><div className="hidden h-7 w-px bg-border sm:block" /><div className="flex size-9 items-center justify-center rounded-full bg-primary text-xs font-bold text-primary-foreground">AP</div></div></header>
      {mobileNav && <div className="border-b border-border bg-card px-5 py-3 lg:hidden"><div className="flex flex-col gap-1">{nav.map(({ label, icon: Icon }) => <button key={label} onClick={() => { selectMenu(label); setMobileNav(false) }} className={`flex items-center gap-3 rounded-lg px-3 py-2 text-sm ${active === label ? 'bg-primary text-primary-foreground' : 'text-muted-foreground'}`}><Icon size={17} />{label}</button>)}</div></div>}      <main className="mx-auto max-w-[1440px] px-5 py-7 md:px-8 md:py-9">{active === 'Rekam Medis' && <section className="mb-7 rounded-2xl border border-border bg-card shadow-sm"><div className="flex items-start justify-between gap-4 border-b border-border p-5 md:p-6"><div><p className="text-sm font-medium text-primary">Data database Neon</p><h2 className="mt-1 text-2xl font-bold tracking-tight">Rekam medis</h2><p className="mt-1 text-sm text-muted-foreground">Riwayat diagnosis dan tindakan pasien dari database klinik.</p></div><button onClick={() => void loadMedicalRecords()} className="rounded-xl border border-border px-3 py-2 text-sm font-semibold hover:bg-muted">Muat ulang</button></div>{medicalLoading ? <p className="p-8 text-sm text-muted-foreground">Memuat rekam medis...</p> : medicalError ? <p role="alert" className="p-8 text-sm text-destructive">{medicalError}</p> : medicalRecords.length === 0 ? <p className="p-8 text-sm text-muted-foreground">Belum ada rekam medis di database.</p> : <div className="overflow-x-auto"><table className="w-full min-w-[900px] text-left text-sm"><thead className="bg-muted/50 text-xs text-muted-foreground"><tr><th className="px-5 py-3">Tanggal</th><th className="px-5 py-3">Pasien</th><th className="px-5 py-3">Diagnosis</th><th className="px-5 py-3">Tindakan</th><th className="px-5 py-3">Dokter</th><th className="px-5 py-3">Catatan</th></tr></thead><tbody>{medicalRecords.map((record) => <tr key={record.id} className="border-t border-border"><td className="px-5 py-4 whitespace-nowrap text-muted-foreground">{new Date(record.visitDate).toLocaleDateString('id-ID')}</td><td className="px-5 py-4 font-semibold">{record.patientName ?? 'Pasien'}</td><td className="px-5 py-4">{record.diagnosis}</td><td className="px-5 py-4">{record.treatment}</td><td className="px-5 py-4 text-muted-foreground">{record.doctor}</td><td className="max-w-[220px] px-5 py-4 text-muted-foreground">{record.notes || '—'}</td></tr>)}</tbody></table></div>}</section>}{active === 'Pasien' && <section className="mb-7 rounded-2xl border border-border bg-card shadow-sm"><div className="flex items-start justify-between gap-4 border-b border-border p-5 md:p-6"><div><p className="text-sm font-medium text-primary">Data database Neon</p><h2 className="mt-1 text-2xl font-bold tracking-tight">Daftar pasien</h2><p className="mt-1 text-sm text-muted-foreground">Seluruh pasien yang tersimpan di database klinik.</p></div><button onClick={() => void loadPatients()} className="rounded-xl border border-border px-3 py-2 text-sm font-semibold hover:bg-muted">Muat ulang</button></div>{patientsLoading ? <p className="p-8 text-sm text-muted-foreground">Memuat data pasien...</p> : patientsError ? <p role="alert" className="p-8 text-sm text-destructive">{patientsError}</p> : patients.length === 0 ? <p className="p-8 text-sm text-muted-foreground">Belum ada pasien di database.</p> : <div className="overflow-x-auto"><table className="w-full min-w-[700px] text-left text-sm"><thead className="bg-muted/50 text-xs text-muted-foreground"><tr><th className="px-5 py-3">Pasien</th><th className="px-5 py-3">NIK</th><th className="px-5 py-3">Tanggal lahir</th><th className="px-5 py-3">Telepon</th><th className="px-5 py-3">Terdaftar</th></tr></thead><tbody>{patients.map((patient) => <tr key={patient.id} className="border-t border-border"><td className="px-5 py-4 font-semibold">{patient.name}</td><td className="px-5 py-4 text-muted-foreground">{patient.nationalId.startsWith('PHONE-') ? '—' : patient.nationalId}</td><td className="px-5 py-4 text-muted-foreground">{new Date(patient.birthDate).toLocaleDateString('id-ID')}</td><td className="px-5 py-4 text-muted-foreground">{patient.phone}</td><td className="px-5 py-4 text-muted-foreground">{new Date(patient.createdAt).toLocaleDateString('id-ID')}</td></tr>)}</tbody></table></div>}</section>}{active === 'Jadwal Dokter' && <section className="mb-7 rounded-2xl border border-border bg-card p-5 shadow-sm md:p-6"><div className="flex items-start justify-between gap-4"><div><p className="text-sm font-medium text-primary">Data database Neon</p><h2 className="mt-1 text-2xl font-bold tracking-tight">Jadwal dokter</h2><p className="mt-1 text-sm text-muted-foreground">Jadwal praktik aktif dari database klinik.</p></div><button onClick={() => void loadDoctorSchedules()} className="rounded-xl border border-border px-3 py-2 text-sm font-semibold hover:bg-muted">Muat ulang</button></div>{doctorLoading ? <p className="mt-8 text-sm text-muted-foreground">Memuat jadwal dokter...</p> : doctorError ? <p role="alert" className="mt-8 text-sm text-destructive">{doctorError}</p> : doctorSchedules.length === 0 ? <p className="mt-8 text-sm text-muted-foreground">Belum ada jadwal dokter.</p> : <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">{doctorSchedules.map((item) => <article key={item.id} className="rounded-xl border border-border bg-background p-4"><div className="flex items-start justify-between gap-3"><div className="flex items-center gap-3"><div className="flex size-11 items-center justify-center rounded-full bg-primary/10 text-sm font-bold text-primary">{item.name.replace('dr. ', '').split(' ').map((part) => part[0]).join('').slice(0, 2)}</div><div><h3 className="font-semibold">{item.name}</h3><p className="text-xs text-muted-foreground">{item.specialty}</p></div></div><span className="rounded-full bg-primary/10 px-2 py-1 text-[11px] font-medium text-primary">Aktif</span></div><div className="mt-4 flex items-center justify-between border-t border-border pt-3 text-sm"><span className="text-muted-foreground">{['Minggu', 'Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat', 'Sabtu'][item.dayOfWeek]}</span><span className="font-semibold">{item.startTime.slice(0, 5)} – {item.endTime.slice(0, 5)}</span></div><p className="mt-2 text-xs text-muted-foreground">{item.room}</p></article>)}</div>}</section>}{active === 'Pendaftaran' && <section className="mb-7 rounded-2xl border border-border bg-card shadow-sm"><div className="flex items-center justify-between border-b border-border p-5"><div><p className="text-sm font-medium text-primary">Data database Neon</p><h2 className="mt-1 text-2xl font-bold tracking-tight">Pendaftaran pasien</h2><p className="mt-1 text-sm text-muted-foreground">Data nyata yang tersimpan di database klinik.</p></div><button onClick={() => void loadRegistrations()} className="rounded-xl border border-border px-3 py-2 text-sm font-semibold hover:bg-muted">Muat ulang</button></div>{registrationsLoading ? <p className="p-8 text-sm text-muted-foreground">Memuat data pendaftaran...</p> : registrationsError ? <p role="alert" className="p-8 text-sm text-destructive">{registrationsError}</p> : registrations.length === 0 ? <p className="p-8 text-sm text-muted-foreground">Belum ada data pendaftaran di database.</p> : <div className="overflow-x-auto"><table className="w-full min-w-[760px] text-left text-sm"><thead className="bg-muted/50 text-xs text-muted-foreground"><tr><th className="px-5 py-3">Antrean</th><th className="px-5 py-3">Pasien</th><th className="px-5 py-3">Poli</th><th className="px-5 py-3">Dokter</th><th className="px-5 py-3">Tanggal</th><th className="px-5 py-3">Status</th></tr></thead><tbody>{registrations.map((item) => <tr key={item.id} className="border-t border-border"><td className="px-5 py-4 font-bold text-primary">A-{String(item.queueNumber).padStart(2, '0')}</td><td className="px-5 py-4"><p className="font-semibold">{item.patientName ?? 'Pasien'}</p><p className="text-xs text-muted-foreground">{item.phone ?? '—'}</p></td><td className="px-5 py-4">{item.clinic}</td><td className="px-5 py-4 text-muted-foreground">{item.doctor}</td><td className="px-5 py-4 text-muted-foreground">{new Date(item.visitDate).toLocaleDateString('id-ID')}</td><td className="px-5 py-4"><span className="rounded-full bg-amber-50 px-2.5 py-1 text-xs font-medium text-amber-700">{item.status}</span></td></tr>)}</tbody></table></div>}</section>}<div className="mb-7 flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="mb-2 text-sm font-medium text-primary">{active === 'Dashboard' ? 'Ringkasan hari ini' : ''}</p><h2 className="text-3xl font-bold tracking-tight">{active === 'Dashboard' ? 'Dashboard klinik' : ''}</h2><p className="mt-1 text-sm text-muted-foreground"></p></div>{active === 'Dashboard' && <><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><StatCard label="Total pasien hari ini" value="128" change="+12.5%" icon={UsersRound} tone="bg-teal-50 text-teal-700" /><StatCard label="Menunggu dilayani" value="24" change="+4.2%" icon={Clock3} tone="bg-amber-50 text-amber-700" /><StatCard label="Dokter aktif" value="8" change="+1" icon={Stethoscope} tone="bg-sky-50 text-sky-700" /><StatCard label="Pendapatan hari ini" value="Rp 8,4 jt" change="+8.7%" icon={Activity} tone="bg-violet-50 text-violet-700" /></div>
