...
export default function ClinicDashboard() {
  ...
  async function submitRegistration(event: React.FormEvent<HTMLFormElement>) {
    ...
    const values = Array.from(form.querySelectorAll('input, select, textarea')).map((field) => (field as HTMLInputElement).value)
    const [name, birthDate, gender, nationalId, phone, clinic, doctor, visitDate, complaint] = values
    const payload = { name, birthDate, nationalId, phone, clinic, doctor, visitDate, complaint }
    ...
    setSaving(false)
    if (!response.ok) { setError('Data belum lengkap atau gagal disimpan.'); return }
    setRegistered(true)
  }
