...
const registrationSchema = z.object({
  ...
  nationalId: z.string().trim().min(5).max(32),
...
export async function POST(request: Request) {
  try {
    ...
    const [registration] = await db.insert(registrations).values({ ...data, patientId: patient.id, queueNumber }).returning({ id: registrations.id, queueNumber: registrations.queueNumber })
